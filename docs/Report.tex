...
\section{Q4: Comparative Analysis with Large Language Models (LLMs)}

This analysis focuses on comparing the performance of the optimized BERT-based architecture against a modern Large Language Model (LLM). The fundamental objective is to evaluate whether a general-purpose generative model, operating solely in inference mode, is capable of competing with or surpassing the effectiveness of a specialized encoder that has undergone fine-tuning.
\subsection{Methodology: Generative Inference}

For this comparison, we selected \texttt{gemma-3-4b-it}, an instruction-tuned model from Google's Gemma family with approximately 4 billion parameters. Unlike the BERT implementation, where the model weights were updated via backpropagation to minimize cross-entropy loss (fine-tuning), the LLM was employed using a \textbf{Few-Shot Prompting} strategy via the Google GenAI API.

The experimental setup was defined as follows:
\begin{itemize}
    \item \textbf{Prompt Engineering:} The model was explicitly instructed to act as a "sentiment analysis expert".
    \item \textbf{Few-Shot Context:} To guide the model's reasoning without weight updates, three labeled examples (one for each class: Positive, Negative, Neutral) were provided in the prompt context window before the target query.
    \item \textbf{Output Constraints:} The generation was restricted to output single-word labels mapped to our numeric classes ($0, 1, 2$).
    \item \textbf{Evaluation Subset:} Due to API rate limits and latency constraints, the evaluation was conducted on a representative subset of 1,000 samples from the test set.
\end{itemize}

\subsection{Performance Evaluation}

We compare the generative capabilities of Gemma against the \texttt{BERT Linear Head} model, which yielded the highest performance among the BERT configurations tested in the previous section.

\subsubsection{LLM Results (Gemma-3-4b-it)}
Fig. \ref{fig:gemma_res} presents the classification report for the generative model. Gemma achieved an \textbf{Accuracy of 0.632} and a \textbf{Macro F1 of 0.630}.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{images/Q4/gemma_report.png}
  \caption{Performance Report for Gemma-3-4b-it (Few-Shot Inference). The model shows a balanced but moderate performance, with notable confusion between polarized sentiments and the neutral class.}
  \label{fig:gemma_res}
\end{figure}

The confusion matrix reveals a specific weakness: a tendency to misclassify polarized tweets (Positive/Negative) as Neutral. This behavior is often attributed to the safety alignment (RLHF) of instruction-tuned models, which biases them towards neutral or non-committal responses when facing ambiguity.

\subsubsection{BERT-based Results}
Fig. \ref{fig:bert_best} displays the performance of the fine-tuned \texttt{BERT Linear Head}. This model achieved an \textbf{Accuracy of 0.664} and a \textbf{Macro F1 of 0.666}.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{images/Q4/bert_best_report.png}
  \caption{Performance Report for the BERT Linear Head Baseline.}
  \label{fig:bert_best}
\end{figure}

The BERT model demonstrates a stronger diagonal in the confusion matrix, indicating superior discriminative power for this specific data distribution compared to the few-shot LLM.

\subsection{Discussion: Specialization vs. Generalization}

The comparison between the specialized encoder (BERT) and the generalist decoder (Gemma) offers valuable insights, summarized in Table \ref{tab:llm_vs_bert}.

\begin{table}[htbp]
\caption{Head-to-Head: Fine-Tuned Encoder vs. Few-Shot LLM}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Model} & \textbf{Params} & \textbf{Method} & \textbf{Macro F1} \\
\hline
\textbf{BERT (Linear)} & \textbf{110M} & \textbf{Fine-Tuning} & \textbf{0.666} \\
\hline
...
\begin{enumerate}
    \item \textbf{Performance Gap:} The fine-tuned BERT model outperforms the LLM by approximately \textbf{3.6\% in Macro F1}. This confirms that, for this dataset, a smaller model adapted to the specific domain (via fine-tuning) is more effective than a massive model relying on general knowledge (few-shot).
    
    \item \textbf{Resource Efficiency:} The efficiency gap is substantial. The BERT model (110M parameters) can be deployed for inference with low latency on standard hardware. In contrast, the LLM (4B parameters) requires significant VRAM or API dependence, with observed latencies in the range of seconds per query.
...
\section{Q6-BERT Architecture and Theoretical Framework}

The Bidirectional Encoder Representations from Transformers (BERT) represents a paradigm shift in language representation learning. Unlike previous models that relied on unidirectional processing (e.g., OpenAI GPT) or shallow concatenation of independent left-to-right and right-to-left models (e.g., ELMo), BERT is designed to pre-train deep bidirectional representations from unlabeled text by jointly conditioning on both left and right contexts in all layers \cite{b_bert}.

\subsection{Model Architecture}

BERT’s architecture is a multi-layer bidirectional Transformer encoder based on the original implementation described by Vaswani et al. \cite{b_att}. The model is composed of a stack of $L$ identical layers (Transformer blocks).

Devlin et al. define two primary model sizes to evaluate the effect of model capacity \cite{b_bert}:
\begin{itemize}
    \item \textbf{BERT\textsubscript{BASE}:} Defined with $L=12$ layers, a hidden size of $H=768$, and $A=12$ self-attention heads, totaling approximately 110 million parameters. This size was chosen to be comparable to OpenAI GPT for fair evaluation.
    \item \textbf{BERT\textsubscript{LARGE}:} A significantly deeper and wider network with $L=24$, $H=1024$, and $A=16$, resulting in approximately 340 million parameters.
\end{itemize}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{images/Q6/bert_arch_comparison.jpg}
  \caption{Differences in pre-training model architectures. BERT uses a bidirectional Transformer, distinguishing it from OpenAI GPT and ELMo \cite{b_bert}.}
  \label{fig:bert_arch}
\end{figure}
Within each layer, the architecture employs two primary sub-layers: a Multi-Head Self-Attention mechanism and a Position-wise Feed-Forward Network. Crucially, a residual connection is employed around each of these two sub-layers, followed by layer normalization.

\subsubsection{Multi-Head Self-Attention}
This mechanism allows the model to jointly attend to information from different representation subspaces at different positions. The attention function is computed on a set of queries ($Q$), keys ($K$), and values ($V$) packed into matrices. The output is a scaled dot-product attention:
...
where $d_k$ is the dimension of the key vectors, serving as a scaling factor to prevent vanishing gradients in the softmax function.

\subsubsection{Feed-Forward Network and GELU Activation}
Following the attention sub-layer, the output is processed by a fully connected Feed-Forward Network (FFN) applied to each position separately and identically. This consists of two linear transformations with a non-linear activation function. A distinguishing feature of BERT, compared to the original Transformer which used ReLU, is the adoption of the Gaussian Error Linear Unit (GELU). The FFN is formulated as:
...
where $W_1$ projects the hidden state from $H$ to an intermediate dimension (3072 for \texttt{BERT\textsubscript{BASE}}), and $W_2$ projects it back to $H$. The GELU activation, defined as $x\Phi(x)$ (where $\Phi(x)$ is the standard Gaussian cumulative distribution function), provides a smoother non-linearity that aids optimization in deep architectures.

\subsubsection{Residual Connections and Normalization}
To enable the training of deep networks, the output of each sub-layer is stabilized using residual connections and Layer Normalization (LayerNorm):
...
Unlike Batch Normalization, Layer Normalization computes statistics across the feature dimension for each token independently, making it invariant to batch size and sequence length.

\subsection{Input Representation}

To handle a wide variety of downstream tasks, BERT requires a flexible input representation capable of handling both single sentences and sentence pairs.

\subsubsection{WordPiece Tokenization}
BERT utilizes a WordPiece vocabulary of 30,000 tokens. This tokenization strategy mitigates the out-of-vocabulary (OOV) problem by decomposing rare words into sub-word units (e.g., "playing" $\rightarrow$ \texttt{play} + \texttt{\#\#ing}).

\subsubsection{Embedding Summation}
The input representation for a given token is constructed by summing three distinct embeddings:
\begin{equation}
\mathbf{E} = \mathbf{E}_{token} + \mathbf{E}_{segment} + \mathbf{E}_{position}.
\label{eq:embeddings}
\end{equation}
\begin{itemize}
    \item \textbf{Token Embeddings:} Vector representations of the WordPiece tokens.
    \item \textbf{Segment Embeddings:} Learned vectors indicating whether a token belongs to sentence A or sentence B.
    \item \textbf{Position Embeddings:} Learned vectors injecting absolute positional information, necessary due to the permutation-invariance of self-attention.
\end{itemize}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{images/Q6/bert_input_rep.jpg}
  \caption{BERT input representation. The input embeddings are the sum of the token embeddings, the segmentation embeddings, and the position embeddings \cite{b_bert}.}
  \label{fig:bert_input}
\end{figure}

Every sequence starts with a special classification token (\texttt{[CLS]}). The final hidden state corresponding to this token ($C \in \mathbb{R}^H$) is used as the aggregate sequence representation for classification tasks. A separator token (\texttt{[SEP]}) is used to distinguish between sequences.

\subsection{Pre-training Objectives}

BERT is pre-trained on two unsupervised tasks using the BooksCorpus (800M words) and English Wikipedia (2,500M words).

\subsubsection{Masked Language Model (MLM)}
To achieve deep bidirectionality without allowing tokens to "see themselves" in multi-layered contexts, BERT masks 15\% of the input tokens at random. The objective is to predict the original vocabulary id of the masked word based only on its context. To mitigate the mismatch between pre-training and fine-tuning (where the \texttt{[MASK]} token does not appear), the training data generator employs the following strategy for the selected tokens:
\begin{itemize}
    \item 80\% of the time: Replace with the \texttt{[MASK]} token.
...
Many downstream tasks (e.g., QA and NLI) require understanding the relationship between two sentences. To model this, BERT is trained on a binary classification task to predict whether a sentence $B$ is the actual next sentence that follows sentence $A$ (IsNext) or a random sentence from the corpus (NotNext).

\subsection{Application Strategies}

As demonstrated in the ablation studies by Devlin et al., BERT can be applied to downstream tasks in two ways: \textit{fine-tuning}, where all parameters are updated end-to-end, and the \textit{feature-based approach}, where fixed features are extracted from specific layers.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.85\linewidth]{images/Q6/bert_ner_table.jpg}
  \caption{Feature-based approach results on CoNLL-2003 NER. Concatenating the last 4 hidden layers yields performance comparable to fine-tuning \cite{b_bert}.}
  \label{fig:bert_table}
\end{figure}

Experimental results shown in Fig. \ref{fig:bert_table} indicate that concatenating the token representations from the top four hidden layers yields performance competitive with fine-tuning (96.1 F1 vs 96.4 F1). In our theoretical framework, this justifies the use of extracted embeddings as robust inputs for downstream classifiers.
